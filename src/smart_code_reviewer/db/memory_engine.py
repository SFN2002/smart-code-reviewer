...
class AsyncMemoryEngine:
    def __init__(self, db_path: str = "smart_code_reviewer.db"):
    ...
    async def get_all_vectors(self) -> List[VectorRecord]:
        async with self._lock:
            if self._pool is None:
                return []
            cursor = await self._pool.execute("SELECT * FROM vectors")
            rows = await cursor.fetchall()
            results = []
            for row in rows:
                embedding = pickle.loads(base64.b64decode(row[1]))
