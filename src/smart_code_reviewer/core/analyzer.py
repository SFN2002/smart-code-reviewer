...
class DeepSignalExtractor(AbstractSignalExtractor, metaclass=RegistryMeta):
    def __init__(self, syntax_dim: int = 512, dataflow_dim: int = 512, intent_dim: int = 256, use_neural: bool = False):
    ...
    def extract_intent(self, source: str) -> np.ndarray:
        if self.use_neural and self._model:
            ...
            if cls_emb.size > self.intent_dim:
                cls_emb = cls_emb[:self.intent_dim]
            elif cls_emb.size < self.intent_dim:
                cls_emb = np.pad(cls_emb, (0, self.intent_dim - cls_emb.size))
            return np.tanh(cls_emb @ self._intent_kernel + self._intent_bias)
        tree = ast.parse(source)
